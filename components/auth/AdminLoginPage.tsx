...
const LoginPage: React.FC = () => {
  ...
  useEffect(() => {
    const savedAttempts = localStorage.getItem('admin_loginAttempts');
    const savedLockout = localStorage.getItem('admin_lockoutUntil');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      localStorage.setItem('admin_loginAttempts', newAttempts.toString());

      if (newAttempts >= 5) {
        const lockoutTime = Date.now() + 60000; // 1 minute
        setLockoutUntil(lockoutTime);
        localStorage.setItem('admin_lockoutUntil', lockoutTime.toString());
