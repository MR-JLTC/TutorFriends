  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm p-0 sm:p-4">
      <div className="bg-white w-full sm:max-w-md rounded-t-2xl sm:rounded-2xl shadow-2xl flex flex-col max-h-[92dvh] sm:max-h-[90vh]">
        ...
        {/* Header */}
        <div className="relative bg-gradient-to-r from-primary-600 to-primary-700 px-5 py-4">
          ...
          <button onClick={onClose} className="absolute right-4 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-white/15 hover:bg-white/25 flex items-center justify-center transition-colors">
        ...
        {(bookingContext?.currentDate || bookingContext?.currentTime) && (
          <div className="mx-5 mt-4 p-3 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-2.5">
        ...
        <div className="px-5 py-4 space-y-4 overflow-y-auto flex-1">
          <div className="grid grid-cols-2 gap-3">
            ...
            {/* Time */}
            <div className="space-y-2">
              <div className="flex items-center justify-between min-h-[1.5rem] gap-1">
                <label className="flex items-center gap-2 text-sm font-semibold text-slate-700">
                ...
                {availableTimeSlots.length > 0 && !loadingBookings && (
                  <span className="text-xs font-normal text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full flex-shrink-0">{availableTimeSlots.length} slots</span>
                )}
                {loadingBookings && (
              ...
              {date && dayBlocks.length > 0 && (() => {
                if (time) {
                  ...
                  if (block) return <div className="text-xs text-slate-600"><span className="font-semibold">Available:</span> {block.start_time} - {block.end_time}</div>;
                }
                return dayBlocks.map((b: any, i: number) => (
                  <div key={i} className="text-xs text-slate-600"><span className="font-semibold">Available:</span> {b.start_time} - {b.end_time}</div>
                ));
              })()}
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              rows={3}
              className="w-full border-2 border-slate-300 bg-white hover:border-slate-400 px-3 sm:px-4 py-2.5 sm:py-3.5 rounded-lg sm:rounded-xl focus:ring-4 focus:ring-slate-200 focus:border-slate-500 transition-all text-sm sm:text-base font-medium text-slate-700 resize-none"
            />
          </div>
        </div>
        ...
        {isSameAsOriginal && (
          <div className="mx-5 mb-3 flex items-start gap-2.5 bg-amber-50 border border-amber-300 rounded-xl px-3.5 py-3">
        ...
        {/* Footer */}
        <div className="px-5 pb-5 pt-2 flex gap-2.5 flex-shrink-0 border-t border-slate-100" style={{ paddingBottom: 'max(1.25rem, env(safe-area-inset-bottom, 1.25rem))' }}>
          <button onClick={onClose} className="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 text-slate-600 font-semibold text-sm hover:bg-slate-50 transition-colors">
            Cancel
          </button>
          <button
            onClick={submit}
